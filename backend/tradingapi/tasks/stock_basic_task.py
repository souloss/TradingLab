...
async def update_stock_basic_info():
    async with get_session_with_ctx() as session:
        ...
        for _, stock in all_stocks.iterrows():
            stock_detail = await stock_fetcher.get_stock_basic_info(stock['交易所'], stock['证券代码'])
            if not stock_detail:
                continue
            ...
            stock_basic_info = StockBasicInfo(
                ...
                listing_date=datetime.fromisoformat(stock_detail.get("上市时间")).date() if stock_detail.get("上市时间") else "",
            ...
            await repo.upsert(stock_basic_info, conflict_columns=[StockBasicInfo.symbol])
