...
def prepare_data(ticker, start_date, end_date):
    ...
    data['Stoch_%K'], data['Stoch_%D'] = talib.STOCH(data['High'], data['Low'], data['Close'])
...
def create_features_and_labels(data, return_threshold=0.05):
    ...
    """
    # 特征集 - 选择最重要的特征
    features = data[['SMA_10', 'SMA_20', 'SMA_50', 
                     'EMA_12', 'EMA_26',
                     'ADX', 'MACD', 'MACD_signal',
                     'RSI', 'Stoch_%K', 'Stoch_%D', 'Momentum',
                     'ATR', 'Bollinger_Position',
                     'OBV', 'Volume_Change',
                     'SMA_Crossover', 'MACD_Crossover',
                     'Bollinger_Position', 'RSI_Overbought', 'RSI_Oversold',
                     'Return_lag_1', 'Return_lag_2', 'Return_lag_3',
                     'Volume_lag_1', 'Volume_lag_2']].copy()
    
    # 创建标签 - 基于未来收益
    data['Label'] = np.where(data['Future_Return'] > return_threshold, 1,  # 买入信号
                   np.where(data['Future_Return'] < -return_threshold, -1, 0))  # 卖出信号
...
class AIOptimizedStrategy(Strategy):
    ...
    def init(self):
        ...
        self.sell_signals = self.I(lambda: np.zeros(len(self.data)), name='sell_signals')
    ...
    def next(self):
        ...
        if not self.position:
            if buy_signal > 0.7:  # 强买入信号
                ...
                # 计算止盈
                take_profit = self.data.Close[-1] + self.take_profit_multiplier * risk_per_share
...
def find_best_entry_exit(data, predictions):
    ...
    for idx, row in buy_points.iterrows():
        ...
        if not next_sell.empty:
            ...
            max_drawdown = (min_price - buy_price) / buy_price * 100 if min_price < buy_price else 0
    ...
    # 按夏普比率排序 (收益率/回撤)
    trades_df['Sharpe_Ratio'] = trades_df['Return_Pct'] / (abs(trades_df['Max_Drawdown']) + 1e-5)
    best_by_sharpe = trades_df.sort_values('Sharpe_Ratio', ascending=False).iloc[0]
    
    # 按持有期排序
    best_by_hold_period = trades_df.sort_values(['Return_Pct', 'Hold_Period'], 
...
if __name__ == "__main__":
    ...
    if best_points:
        print("\n===== 最佳买卖点分析 =====")
        print(f"按收益率最佳: 买入 {best_points['by_return']['Buy_Date'].date()}, "
              f"卖出 {best_points['by_return']['Sell_Date'].date()}, "
              f"收益: {best_points['by_return']['Return_Pct']:.2f}%")
        
        print(f"按风险回报比最佳: 买入 {best_points['by_sharpe']['Buy_Date'].date()}, "
              f"卖出 {best_points['by_sharpe']['Sell_Date'].date()}, "
              f"夏普比率: {best_points['by_sharpe']['Sharpe_Ratio']:.2f}")
        
        print(f"按持有期最佳: 买入 {best_points['by_hold_period']['Buy_Date'].date()}, "
    ...
    if best_points:
        # 标记最佳买卖点
        plt.scatter(best_points['by_return']['Buy_Date'], 
                    stock_data.loc[best_points['by_return']['Buy_Date'], 'Close'],
                    marker='*', color='blue', s=200, label='最佳买入(收益)')
        plt.scatter(best_points['by_return']['Sell_Date'], 
                    stock_data.loc[best_points['by_return']['Sell_Date'], 'Close'],
                    marker='*', color='purple', s=200, label='最佳卖出(收益)')
        
        plt.scatter(best_points['by_sharpe']['Buy_Date'], 
                    stock_data.loc[best_points['by_sharpe']['Buy_Date'], 'Close'],
                    marker='*', color='cyan', s=200, label='最佳买入(风险回报)')
        plt.scatter(best_points['by_sharpe']['Sell_Date'], 
                    stock_data.loc[best_points['by_sharpe']['Sell_Date'], 'Close'],
                    marker='*', color='magenta', s=200, label='最佳卖出(风险回报)')
    ...
    class BacktestStrategy(AIOptimizedStrategy):
        def init(self):
            super().init()
            self.df = self.data.df
            self.buy_signals = self.I(lambda: self.df['Buy_Prob'].values, name='buy_signals')
            self.sell_signals = self.I(lambda: self.df['Sell_Prob'].values, name='sell_signals')
